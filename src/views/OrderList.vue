...
/*
  ...
  The rest of the file expects a reactive `products` variable,
  so expose `products` as a computed wrapper around productStore.products
*/
...
const filteredOrders = computed(() => {
    ...
    currentPage.value =
        totalPages.value < currentPage.value ? 1 : currentPage.value;
...
const createPDF = (currentOrder) => {
    ...
    doc.text(
        "Banadevi Patna, Kabisurya Nagar, Ganjam",
        10,
        60 + 2 * lineSpacing,
    );
    ...
    if (invoiceItems && typeof invoiceItems === "object") {
        ...
        if (Array.isArray(invoiceItems)) {
            ...
            const orderArray = Object.values(
                invoiceItemcurrentOrder?.orderDates,
            );
                    ...
                    <footer>
                        <div class="grid">
                            <button
                                ...
                                :disabled="isSaveButtonDisabled"
                                :aria-invalid="
                                    isSaveButtonDisabled ? 'true' : false
                                "
                            >
                                Save
                            </button>
