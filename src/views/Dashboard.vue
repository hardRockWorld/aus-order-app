    ...
    <div class="dashboard">
        ...
        <div class="metrics-container">
            <div class="metric-card">
                ...
                <div class="metric-change">
                    <span v-if="orderGrowth > 0" class="positive"
                        >↑ {{ orderGrowth }}%</span
                    >
                    <span v-else-if="orderGrowth < 0" class="negative"
                        >↓ {{ Math.abs(orderGrowth) }}%</span
                    >
            ...
            <div class="metric-card">
                ...
                <div class="metric-description">
                    {{
                        orders.length > 0
                            ? Math.round(
                                  (pendingOrdersLength / orders.length) * 100,
                              )
                            : 0
                    }}% of total orders
                </div>
            ...
            <div class="metric-card">
                ...
                <div class="metric-description">
                    {{
                        orders.length > 0
                            ? Math.round(
                                  (completedOrdersCount / orders.length) * 100,
                              )
                            : 0
                    }}% of total orders
                </div>
        ...
        <div class="lists-container">
            ...
            <div class="list-card">
                ...
                <div class="list-body">
                    <div
                        v-for="order in lastPaymentDetails.slice(0, 5)"
                        :key="order.sln"
                        class="list-item"
                    >
                        <div class="list-item-content">
                    ...
                    <div
                        v-if="lastPaymentDetails.length === 0"
                        class="empty-list"
                    >
...
const calculateOrderGrowth = (timeInterval) => {
    ...
    const previousPeriodOrders = orders.value.filter(
        (order) =>
            new Date(order.orderDate).getTime() >= previousPeriodStart &&
            new Date(order.orderDate).getTime() < timeInterval,
    );
...
const updateOrdersData = () => {
    ...
    // Clear the chartData
    const { data, filteredOrders } = getChartData(
    ...
    pendingOrders.value = orders.value.filter(
        (item) =>
            new Date(item.orderDate).getTime() >= timeInterval &&
            item.status === "placed",
    );
...
const getChartData = (interval, startTime) => {
    ...
    const filteredOrders = orders.value.filter((order) => {
        if (new Date(order.orderDate).getTime() >= startTime) {
            ...
            switch (interval) {
                case "weekly":
                    dateKey = new Date(order.orderDate).toLocaleDateString(
                        "en-US",
                        { weekday: "short" },
                    );
                    break;
                case "monthly":
                    ...
                    break;
                case "quarterly":
                    dateKey = new Date(order.orderDate).toLocaleString(
                        "en-US",
                        { month: "long" },
                    );
                    break;
...
.metric-title {
    font-size: 15px;
    color: var(--primary); /* theme primary for card titles */
    margin-bottom: 8px;
    font-weight: 700;
...
.metric-value {
    ...
    color: var(
        --primary
    ); /* use theme primary for metric values (dark mode override remains in file) */
    margin-bottom: 4px;
    letter-spacing: -0.5px;
}
...
.list-title {
    font-size: 16px;
    font-weight: 700;
    color: var(--primary); /* theme primary for list titles */
    margin: 0;
    text-transform: uppercase;
    letter-spacing: 0.5px;
}
...
.customer-name {
    font-weight: 600;
    color: var(--primary); /* theme primary for customer names */
    font-size: 14px;
}
