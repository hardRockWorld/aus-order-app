...
// load products from Firestore into the product store on mount
onMounted(() => {
    productStore.loadProducts();
});

// reactive products array for compatibility with existing components
                    ...
                    <fieldset class="order-item-container">
                        ...
                        <div v-for="(item, i) in order.items" :key="i">
                            <OrderItemRow
                                ...
                                "
                                @update:total-price="
                                    (productName, itemAmount, totalMrpAmount) =>
                                        updateTotalOrderAmt(
                                            productName,
                                            itemAmount,
                                            totalMrpAmount,
                        ...
                        <label for="billAmt">
                            <p>
                                <span style="color: #0a3d0a"
                                    >Total Bill Amount: &#8377;
                                    {{ order.totalBillAmt }}</span
                                >
                                <small
                                    class="notification green strikethrough"
                                    v-if="order.totalBillAmt > 1.0"
                                    >( &#8377;
                                    {{ order.totalMrpBillAmt }})</small
                                >
                            </p>
                        </label>
                    </fieldset>
